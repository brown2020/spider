  ...
  return (
    <>
      {popups.map((popup) => {
        ...
        const comboColor = isCombo ? `hsl(${40 + popup.combo! * 8}, 100%, 55%)` : '#fff';
...
export const NearMissIndicator = memo(function NearMissIndicator({ position, onComplete }: NearMissProps) {
  return (
    <div
...
export const RingBurst = memo(function RingBurst({ position, color, size }: RingBurstProps) {
  const rings = useMemo(() => [
    { delay: 0, scale: 1 },
    { delay: 0.1, scale: 1.5 },
    { delay: 0.2, scale: 2 },
