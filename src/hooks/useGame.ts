...
export const useGame = () => {
  ...
  // Zip to target function
  const zipTo = useCallback((target: { x: number; y: number }) => {
    setGameState((prev) => {
      if (prev.webEnergy < 15) return prev; // Cost for zipping
      ...
      return {
        ...
        velocity,
        isZipping: true,
        isJumping: true, // Treat as jumping for physics
        webEnergy: prev.webEnergy - 15,
      };
    });
  }, []);
  ...
  useEffect(() => {
    ...
    const gameLoop = setInterval(() => {
      setGameState((prev) => {
        ...
        // Apply gravity if jumping and not zipping
        if (
          (newState.isJumping ||
            newState.position.y <
            window.innerHeight - GAME_CONFIG.physics.groundHeight) && !newState.isZipping
        ) {
        ...
        // Stop zipping if we hit something or slow down enough (simplified for now: stop if we hit bounds)
        if (newState.isZipping) {
          // Drag for zipping to eventually stop it if we don't hit anything
          newState.velocity.x *= 0.95;
          newState.velocity.y *= 0.95;

          if (Math.abs(newState.velocity.x) < 1 && Math.abs(newState.velocity.y) < 1) {
            newState.isZipping = false;
          }
        }
        ...
        if (newState.position.x < spiderRadius) {
          newState.position.x = spiderRadius;
          hitWall = true;
        } else if (newState.position.x > window.innerWidth - spiderRadius) {
          newState.position.x = window.innerWidth - spiderRadius;
          hitWall = true;
        }

        if (newState.position.y < spiderRadius) {
          newState.position.y = spiderRadius;
          hitWall = true;
        } else if (newState.position.y > window.innerHeight - GAME_CONFIG.physics.groundHeight) {
          newState.position.y = window.innerHeight - GAME_CONFIG.physics.groundHeight;
          newState.velocity.y = 0;
          newState.isJumping = false;
          newState.isZipping = false; // Stop zipping on ground
        }

        if (hitWall && newState.isZipping) {
          newState.isZipping = false;
      ...
      setPreyList((prevPreyList) => {
        ...
        return prevPreyList.map((prey) => {
          ...
          if (!prey.isTrapped) {
            // Flee behavior
            let fleeVelocity = { ...prey.velocity };
            ...
            if (distanceToSpider < fleeRadius) {
              const angle = Math.atan2(
                prey.position.y - gameState.position.y,
                prey.position.x - gameState.position.x
              );
              const fleeSpeed = GAME_CONFIG.prey.speed * 1.5;
              fleeVelocity.x = Math.cos(angle) * fleeSpeed;
              fleeVelocity.y = Math.sin(angle) * fleeSpeed;
            }

            // Apply flee velocity to prey
            const preyWithFlee = { ...prey, velocity: fleeVelocity };

            const { position, velocity } = updatePreyPosition(preyWithFlee);
  ...
  }, [isPaused, webs, gameState.position]); // Added gameState.position for flee logic
  ...
  useEffect(() => {
    ...
    const spawnPrey = () => {
      ...
      const getRandomSpeed = () => {
        ...
        let randomSpeed = (Math.random() - 0.5) * 2 * speed;
